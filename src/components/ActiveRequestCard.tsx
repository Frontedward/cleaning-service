...
const ActiveRequestCard: React.FC<ActiveRequestCardProps> = ({ request, index }) => {
  const { assignRequest, staff } = useCleaningContext();
  ...
  return (
    <div className="request-card bg-white border-2 border-turquoise-light shadow rounded-lg p-4 mb-4 animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-3">
        ...
        <div className="text-xs text-pink font-bold uppercase">Поддерживающая уборка</div>
      </div>
      ...
      <div className="flex justify-end pt-2">
        <button onClick={handleTake} className="btn bg-pink text-white px-4 py-2 rounded-lg hover:bg-turquoise-dark transition">
